...
namespace SqlFu.DDL.Generators.MySql
{
    internal class MySqlColumnChangesWriter:AbstractColumnChangesWriter
    {
        ...
        {
            Builder.AppendFormat("alter table {0} alter {1} drop default;",MySqlProvider.EscapeIdentifier(col.TableName),MySqlProvider.EscapeIdentifier(col.Name));
        }
        ...
        {
            Builder.AppendFormat("alter table {0} alter {1} set default '{2}';", MySqlProvider.EscapeIdentifier(col.TableName), MySqlProvider.EscapeIdentifier(col.Name),col.DefaultValue);
        }
    }
}