        ...
        public void Run(params IMigrationTask[] tasks)
        {
            ...
            using(var t=_db.BeginTransaction())
            {
                foreach (var task in tasks)
                {
                    ...
                    {
                        _logger.Info("Installing database schema '{1}' with version {0}", task.CurrentVersion,task.SchemaName);
                    }
                    else
                    {
                        _logger.Info("Executing '{2}' migration from version {0} to version {1}", task.CurrentVersion, task.NextVersion,task.SchemaName);
                    }
                    task.Execute(_db);
                }   
                t.Commit();
