...
namespace SqlFu.Migrations
{
    public class SchemaMigrationExecutor:IMigrateSchema
    {
        ...
        public void MigrateToLatestFrom(string currentVersion)
        {
            ...
            _runner.Run(_tasks.Where(t=>t.CurrentVersion.CompareTo(vers)>=0 && (t.NextVersion!=null)).OrderBy(t=>t.CurrentVersion).ToArray());            
        }
        ...
        public void InstallSchema()
        {
            ...
            if (tasks.Count()>1) throw new InvalidOperationException(string.Format("Found more than one migration task used to setup the database schema '{0}'. Each schema must have only one such task",SchemaName));
